...
static void unittest(const void *ptr) {
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    ts_assert_test(byte_should == byte_out, "%c", byte_should, "%c", byte_out,
                   "The Byte at index %d is not equal", i);
  }
...
int main(int argc, char *argv[]) {
  ...
  TestCase cases[] = {{NULL, NULL, 0, NULL},
                      {NULL, "", 0, NULL},
                      {empty_scene, "", 0, NULL},
                      {empty_scene, "./libtheseed_components.so", 0, NULL},
                      {plugin_scene, "", 0, NULL},
                      {plugin_scene, "./libtheseed_components.so",
                       strlen("./libtheseed_components.so") + sizeof(size_t),
                       "\34\0\0\0\0\0\0\0./libtheseed_components.so"}};

  // Constructing Tests

  for (size_t i = 0; i < 6; i++) {
    char *path = g_strdup_printf("/theseed/test_ecs_add_component/%ld", i);
    g_test_add_data_func(path, &cases[i], unittest);
    free(path);
  }

  ts_destroy_scene(empty_scene);
  ts_destroy_scene(plugin_scene);
