...
int main(int argc, char *argv[]) {
  ...
  ts_assert(
      ...
      "Failed to load the plugin");
  TS_Entity entity_with_comp2 = ts_add_entity(scene_with_component2);
  ...
  TestCase cases[] = {
      ...
      {scene_with_component, entity_with_comp, "TS_Transform",
       1}, // Valid component
      {scene_without_component, entity_without_comp, "TS_Transform",
       0},                                             // Non-existent component
      {scene_with_component2, 999, "TS_Transform", 0}, // Invalid entity
  };

  // Constructing Tests
  for (size_t i = 0; i < 5; i++) {
    char *path =
        g_strdup_printf("/theseed/test_ecs_entity_get_component/%ld", i);
